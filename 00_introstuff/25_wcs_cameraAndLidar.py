...
while True:
    ...
    points_in_buffer, colors_in_buffer, wcs_points_camera, camera_origin = redraw_camera(transformation_values['camera']['transformation_matrix'], joint, depth, rgb, principal_point, camera_resolution, bufsize, points_in_buffer,colors_in_buffer,i,bound,box,args.detail)
    if args.lidar:
    ...
    data_collect_wcs = np.vstack([data_collect_wcs, np.concatenate((np.tile(joint.reshape(-1,7), (wcs_points_camera.shape[0],1)), np.tile(camera_origin.reshape(-1,4), (wcs_points_camera.shape[0],1)), wcs_points_camera), axis=1)])
