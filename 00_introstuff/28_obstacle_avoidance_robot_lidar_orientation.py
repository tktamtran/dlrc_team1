...
def prioritization_orientation(dq, ddq, ddx, J, dJ, sigma_ddq, sigma_ddx):
...
def move_to_target_orientation(sigma_ddx_ee, sigma_ddq_null, target_th_null_out, current_j_pos, dgain_null, current_j_vel ):
    ...
    target_j_acc, target_j_sigma = prioritization_orientation(dq=np.array([0] * 7),
                                                  ddq=qd_null,
                                                  ddx=x_dd,
                                                  J=J,
                                                  dJ=np.zeros(J.shape),
    ...
    return target_j_acc, target_j_sigma

def collision_avoidance_j6_orientation(target_j_acc, target_j_sigma):
...
def collision_avoidance_j4_orientation(target_j_acc, target_j_sigma):
...
def move_to_target(sigma_ddx_ee, sigma_ddq_null, target_th_null_out, current_j_pos, dgain_null, current_j_vel ):
    J = SLRobot.Jacobian(np.array([current_j_pos]), 6)  # index of the end-eff link
    J = J[:3]
    qd_null = pgain_null * (target_th_null_out - current_j_pos) - dgain_null * current_j_vel
    x_dd = pgain_ee[:3] * (target_out[:3] - current_pose[:3]) - dgain_ee[:3] * current_vel[:3]
    target_j_acc, target_j_sigma = prioritization(dq=np.array([0] * 7),
                                                  ddq=qd_null,
                                                  ddx=x_dd,
                                                  J=J,
                                                  dJ=np.zeros(J.shape),
                                                  sigma_ddx=sigma_ddx_ee[:3,:3],
                                                  sigma_ddq=sigma_ddq_null)

    return target_j_acc, target_j_sigma


def collision_avoidance_j6(target_j_acc, target_j_sigma):
    n_obs_link = 4  # the index of the obstacle link, link 4: joint 6; link 3: joint 4.
    J = SLRobot.Jacobian(np.array([current_j_pos]), n_obs_link)
    J = J[:3]
    FK_ = SLRobot.FK(np.array([current_j_pos]))
...
def collision_avoidance_j4(target_j_acc, target_j_sigma):
    n_obs_link = 3  # the index of the obstacle link, link 4: joint 6; link 3: joint 4.
    J = SLRobot.Jacobian(np.array([current_j_pos]), n_obs_link)
    J = J[:3]
    FK_ = SLRobot.FK(np.array([current_j_pos]))
...
while True:
    ...
    close_to_target = np.linalg.norm(target_c_pos[:3] - current_c_pos) < 0.05
    if close_to_target:
        print("Orientation")
        # move to target
        target_j_acc, target_j_sigma = move_to_target_orientation(sigma_ddx_ee, sigma_ddq_null, target_th_null_out, current_j_pos,
                                                      dgain_null, current_j_vel)

        # first obstacle
        target_j_acc, target_j_sigma = collision_avoidance_j4_orientation(target_j_acc, target_j_sigma)

        # second obstacle
        target_j_acc, target_j_sigma = collision_avoidance_j6_orientation(target_j_acc, target_j_sigma)
    else:
        print("Position")
        # move to target
        target_j_acc, target_j_sigma = move_to_target(sigma_ddx_ee, sigma_ddq_null, target_th_null_out, current_j_pos, dgain_null, current_j_vel )

        # first obstacle
        target_j_acc, target_j_sigma = collision_avoidance_j4(target_j_acc, target_j_sigma)

        # second obstacle
        target_j_acc, target_j_sigma = collision_avoidance_j6(target_j_acc, target_j_sigma)
    ...
    # org_uff = uff # THIS WAS ADDED MANUALLY
    uff = np.clip(uff, -torque_limit, torque_limit)

    # THIS PART WAS MANUALLY ADDED TO STOP THE ROBOT MOVEMENT FASTER
    # mask = np.sign(uff) + np.sign(current_j_vel)
    # # the parts with unequal sign (where we want to brake) add up to zero)
    # mask_bool = mask == 0
    # # print(f'{mask} \t {mask_bool}')
    # # mask = np.sign(uff) + np.sign(uff_diff)
    # # mask_bool = (mask == 0)
    # # print(f'{mask_bool.shape} \t {mask_bool.dtype}')
    # print(f'{mask} \t {mask_bool}')

    # uff = np.array([org_uff[idx] if mask[idx] else u for idx, u in enumerate(uff)])
    # print(f'{uff} \n {org_uff}')
